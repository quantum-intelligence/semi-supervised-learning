...
# Modified for python3.6 version
#
# Don't know if this is recent or older versino of mag_metrics_functions.py code
#
...
def gen_tally(main_dir, df_mag, recalculate):
    """
        creates df_spin_counts from df_mag
        input: df_mag, recalculate
        output: df_spin_counts
    ...
    for ith, item in enumerate(df_fm_afm['formula_fm']):
        ...
        formula_sorted = formula_set[sortdex]
                     #print(sortdex)
                         #print(formula_sorted)
        edf_sorted = edf_sub.iloc[sortdex,:].copy()
        #edf_sorted = edf_sorted.drop(columns="level_0")
        edf_sorted = edf_sorted.reset_index()
    return edf_sorted
...
def get_df_optima_local(df_master):
    ...
    for c in df_master[:].iterrows():
        ...
        magmoms_Cr = (c[1][['mz_Cr_fm','mz_Cr_afm']])
        magmoms_TM = (c[1][['mz_TM_fm','mz_TM_afm']])
        #print(magmoms_Cr.columns)
        #print('magmoms_Cr', magmoms_Cr,magmoms_Cr[0])
        if (magmoms_Cr[0] < 0) == True: #effectively if Cr_fm is negative
            print('(magmoms_Cr[0] < 0)')
            print(magmoms_Cr[0])
            print(magmoms_TM)
            magmoms_Cr = magmoms_Cr*np.asarray([-1, 1])
            #magmoms_TM = magmoms_TM*np.asarray([-1, 1])
        if (magmoms_Cr[1] < 0) == True: #effectively if Cr_fm is negative
            print('magmoms_Cr[1]')
            print(magmoms_Cr[1])
            print(magmoms_TM)
            magmoms_Cr = magmoms_Cr*np.asarray([1, -1])
            magmoms_TM = magmoms_TM*np.asarray([1, -1])
        if (magmoms_Cr < 0).all() == True:
            print('BOTH spins flipped in FM !! error somewhere')
            print(magmom_Cr)
            magmoms_Cr = magmoms_Cr*np.asarray([-1, -1])
            magmoms_TM = magmoms_TM*np.asarray([-1, -1])
        #    magmoms_Cr = -1*(magmoms_Cr)
        #    magmoms_TM = -1*(magmoms_TM)
        mindex_energy = np.argwhere(energies == np.min(energies))[0][0]
        optimum_mag_Cr = magmoms_Cr[mindex_energy]
        optimum_mag_TM = magmoms_TM[mindex_energy]
        optMag_Cr.append(optimum_mag_Cr)
...
class magnetization:
    ...
    def __init__(self, stem, target):
        """
        - takes a folder of just outCAR files
        - modify to accept a folder of VASP calc with OUTCAR file embedded
        """
        self.stem = stem
        self.target = target
        ...
        self.outs = outs
        #spinlabel = os.path.join(spinlabel,'OUTCAR')
        # if 'OUTCAR' in outs[0]:
        #     self.outs = outs
        # else:
        #     #outs = [os.path.join(x, spinlabel) for x in outs]
        #     #self.outs = [x for x in outs if os.path.exists(x)]
        #     print('Error with OUTCAR data')
    ...
    def gen_formula(self):
        ...
        """
        formula = [x.split('_')[0] for x in self.outs]
        formula = [x.split('/')[0] for x in self.outs]
        return formula
    ...
    def get_mag_flip(self, magarray_x_set, verbose):
        """ pick up any spin flips from magnetization data"""
        # extract magnetization results for relaxed structures
        mag_x_inital = []
        mag_x_final = []
        #spin_flip = []
        for ith, x in enumerate(magarray_x_set):
            if len(x) == 0:
                if verbose == True:
                    print(outs[ith], ' has no magnetic info')
                mag_x_final.append(np.nan)
                mag_x_inital.append(np.nan)
            else:
                init_mag = x[0]
                relaxed_mag = x[-1]
                #flipped = init_mag*relaxed_mag
                #if flipped < 0:
                #    flip = -1  #if product is negative then spin has flipped
                #else:
                #    flip = 1
                #spin_flip.append(flip)
                mag_x_inital.append(init_mag)
                mag_x_final.append(relaxed_mag)
        return mag_x_inital, mag_x_final #, spin_flip


    def spin_flip_df(self,target):
        """
            Create datafraome containing local magnetic moments given target folder
            - get spin flips using: get_mag_flip(self, magarray_x_set, verbose)
            input: target
            output: df_mag
        """
        ...
        mag_x_init, mag_x_final = self.get_mag_flip(magarray_x_set,False)
        mag_y_init, mag_y_final = self.get_mag_flip(magarray_y_set,False)
        mag_z_init, mag_z_final = self.get_mag_flip(magarray_z_set,False)
        #print('mag_x_final',mag_x_final) # array of items

        formula = self.gen_formula()
        mag_z_tots, mag_z_ds = self.mag_tots_gen(mag_z_final, forbit)
        mag_x_tots, mag_x_ds = self.mag_tots_gen(mag_x_final, forbit)
        mag_y_tots, mag_y_ds = self.mag_tots_gen(mag_y_final, forbit)
        #print('mag_tots', mag_tots,'\n') #another array
        mag_z_init, mag_z_dinit = self.mag_tots_gen(mag_z_init, forbit)
        mag_x_init, mag_x_dinit = self.mag_tots_gen(mag_x_init, forbit)
        mag_y_init, mag_y_dinit = self.mag_tots_gen(mag_y_init, forbit)

        mag_z_tots_arr = np.vstack(mag_z_tots)
        mag_x_tots_arr = np.vstack(mag_x_tots)
        mag_y_tots_arr = np.vstack(mag_y_tots)

        mag_z_init_arr = np.vstack(mag_z_init)
        mag_x_init_arr = np.vstack(mag_x_init)
        mag_y_init_arr = np.vstack(mag_y_init)

        mz_CrTM = mag_z_tots_arr[:,:2]
        mx_CrTM = mag_x_tots_arr[:,:2]
        my_CrTM = mag_y_tots_arr[:,:2]
        #print('mz_CrTM ', mz_CrTM)
        mz_CrTM_init = mag_z_init_arr[:,:2]
        mx_CrTM_init = mag_x_init_arr[:,:2]
        my_CrTM_init = mag_y_init_arr[:,:2]
        #print('mz_CrTM ', mz_CrTM_init)
        ...
        mz_Cr_init = reorder(mz_CrTM_init, TM_ions)[:,0]
        mz_TM_init = reorder(mz_CrTM_init, TM_ions)[:,1]
        mx_Cr_init = reorder(mx_CrTM_init, TM_ions)[:,0]
        mx_TM_init = reorder(mx_CrTM_init, TM_ions)[:,1]
        my_Cr_init = reorder(my_CrTM_init, TM_ions)[:,0]
        my_TM_init = reorder(my_CrTM_init, TM_ions)[:,1]

        #print(mz_TM)
        #print(mz_TM_init)
        #print(mz_TM*mz_TM_init)
        spinflips = np.asarray(mz_TM*mz_TM_init)
        spinflip_dex = np.where(spinflips < 0)
        flipped = np.ones(len(spinflips))
        flipped[spinflip_dex] = -1
        #print(flipped)
        df_mag = pd.DataFrame()
    ...
    #magarray_x_set, magarray_y_set, magarray_z_set, forbit = get_magdata(self, outs)
    ...
    def gen_localmag_df(self,stem,target):
        ...
        """
        # stem = '/Users/trevorrhone/Documents/Kaxiras/2DML/Alloys_ML/local_mu/'
        # ext = '/Users/trevorrhone/Documents/Kaxiras/2DML/Alloys_ML/local_mu/afm_OUTCAR_files_Te'
        ext = os.path.join(stem, target)
        outs = os.listdir(ext)
        outs = [x for x in outs if '.' not in x]
        #spinlabel = os.path.join(spinlabel,'OUTCAR')
        # if 'OUTCAR' in outs[0]:
        #     print('folder of OUTCAR files')
        # else:
        #     #outs = [os.path.join(x, spinlabel) for x in outs]
        #     #outs = [x for x in outs if os.path.exists(x)]
        #     print('error with OUTCAR files')
