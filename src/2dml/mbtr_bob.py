...
#NOTES:
# Updated 10-5-2018 to include ref vector for CGT
...
def getMasterMatrix(bag, bond_counts, master_list, bondtypes):
    ...
    max_rowsize = 110 ### NOTE this one                 # Want to maximize this.
...
def bob_update(df2, sigma, min_dist,on_the_fly,p,df_smiles,NUM_flavors,filestem):
    """
    ...
    # get_bob_df(df2b)
    recalculate = True
    filepath = os.path.join(filestem, 'bob_space.p')
...
def get_Bobfeatures_multi(Bob_vector_flavors, sigma, p, ref_vec):
    """
...
def get_bob_multi(df2, sigma, on_the_fly, p, df_smiles, NUM_flavors):
    ...
    # get refence vector for CGT:
    set_zero_ref = False
    index = np.where(df2['formula'] == 'Cr2Ge2Te6')[0]
    if len(index) == 0:
        index = np.where(df2['formula'] == 'Cr2Ge2Se6')[0]
        if len(index) == 0:
            index = np.where(df2['formula'] == 'Cr2Ge2S6')[0]
            if len(index) == 0:
                set_zero_ref = True
            else:
                index = index[0]
        ...
        index = index[0]
    if set_zero_ref == False:
        ref = np.asarray([x[:,index] for x in Bob_vector_flavors])
    else:
        ref = np.asarray([x[:,0] for x in Bob_vector_flavors])
        ref = ref*0.0  #Set all to zeros
    #
    features_flavors = get_Bobfeatures_multi(Bob_vector_flavors, sigma, p, ref)
...
def get_BoBvectors_multi(df2,species, on_the_fly, df_smiles, NUM_flavors):
    ...
    """
    # print('df2.columns', [x for x in df2.columns])
    molecule_list = df2[species]
...
def get_masterlist(molecule_list, on_the_fly, df_smiles, NUM_flavors):
    ...
    print('bondtypes_list', bondtypes_list)
...
def create_smilesbobs(smilesRECALCULATE, df_smiles,sigma, min_dist,on_the_fly,p, filestem):
    """
...
def getMasterMatrix_multi(bag_set, bond_counts, master_list, bondtypes, NUM_flavors):
    ...
    if type(bag) == type(np.nan):
        return np.nan
    #max_rowsize = np.max(bond_counts) # THIS VARIES depending on molecule..
    max_rowsize = 20                  # Want to maximize this.
    # That is, consider for biggest molecule
    max_colsize = len(master_list)
...
def mol_matrix_multi(structure,on_the_fly,df_smiles,NUM_flavors):
    ...
    if pd.isnull(structure) == True:
        ...
        if on_the_fly == True:
            ...
            #OX_bag = [1.0*element(i).dipole_polarizability for i in atom_bag]
            nup = [1.0*element(i).ec.unpaired_electrons() for i in atom_bag]
            dip = [1.0*element(i).dipole_polarizability for i in atom_bag]
            covrad = [1.0*element(i).covalent_radius_pyykko for i in atom_bag]
            nup = np.asarray(nup, dtype='float')
            dip = np.array(dip, dtype='float')
            covrad = np.array(covrad, dtype='float')
            OX_bag = nup
            J = dip*covrad
        else:
            if (df_smiles['cids'] == molecule_name ).any() == False:
            ...
            for j in np.arange(N):
                if j >= i:
                    ...
                    if i == j:
                        ...
                        MolMatrix[5,i,j] = (J[i] + J[j])*OX_bag[i]*OX_bag[j]
                MolMatrix[0,j,i] = MolMatrix[0,i,j]
                MolMatrix[1,j,i] = MolMatrix[1,i,j]
