...
# import error..
#from pymatgen.ext.matproj.rest import MPRester # jon + github for import errors
#
# fprintdata = np.load('fprintdata.npy')
# bandg = np.load('bandg.npy')


def get_unique_elem_info(df, recalculate=False):
    """
        Get all possible elements first, get unique list and call 
        materials project using this..
    """
    main_dir = '/Users/trevorrhone/Documents/Kaxiras/2DML/Alloys_ML'
    os.chdir(main_dir)
    #recalculate = False
    if not os.path.exists('elements_energies.csv') or recalculate:
        ...
        for ith, cmpd in df['formula'].iteritems():
            ...
            #print(mp_cmpd.get_atomic_fraction('Cr'))
            elems = mp_cmpd.elements
            for el in elems:
                elems_list.append(el)
            #print(mp_cmpd.element_composition)
            #print(mp_cmpd)
        ...
        print('df_elements.shape', df_elements.shape)
        print('len elem_min_energy', len(elem_min_energy))
...
def get_atom_counts(df, df_elements):
...
def element_energy_lookup(atom,df_elements):
    ...
    """
    #print(type(df_elements['unique_elem'][0]))
...
# atom = 'Mn'
# atom = mp.Element(atom)
# target_energy = element_energy_lookup(atom, df_elements)
# print(target_energy)
...
def get_atom_energy_total(df, df_elements):
    ...
    for ith, cmpd in df['formula'].iteritems():
        ...
        #print('energy_frac_ratios', energy_frac_ratios)
...
# property_values = nvalence
# sum_dif_list, std_dif_list, prop_std_list = dif_stats_calc(property_values)
# plt.scatter(sum_std_list, prop_std_list)
# plt.scatter(dif_std_list, sum_dif_list,marker='x')
# plt.show()
...
def parse_contcar_data(ext, contdir):
    ...
    for dir in dirs:
        ...
        #print('structure_label',structure_label)
...
def calc_chem_similarity(elements_set, distmatrix_set,p_num,ABweight):
    ...
    for ith, species in enumerate(elements_set):
        ...
        # Dont report chemical hardness here
        #prop_stats = propertyStats(species, 11) #11 for hardness
        #prop_stats_set.append(prop_stats)
