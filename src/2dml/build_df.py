...
def get_ion_shell(atom):
    """
        take into account covalent # of elements in crystal structure
    """
    x = element(atom)
    g_id = x.group_id
    if g_id == None: # Er has None group_id at May 22,2019 where was fine before...
        #print('atom for Er', atom)
        x = mp.Element(atom)
        g_id = x.group
        #print(g_id)
        if (g_id > 2) & (g_id < 13): nion = 3
        if (g_id >= 13) & (g_id <= 15): nion = 3
        if g_id == 16: nion = -2
        x = element(atom)
        lshell = x.ec.ionize(n=nion).last_subshell()
    else:
        #print('got here', atom)
        if (g_id > 2) & (g_id < 13): nion = 3
        if (g_id >= 13) & (g_id <= 15): nion = 3
...
def get_mendeleev(fname):
    ...
    for elem in mpielements:
        ...
        lastsubshell = get_ion_shell(elem) # take into account covelnt # of element in crystal strucure
...
# Code commented out 10-2-2018 Not USEd in main nottebook:
#
...
def scaledata(X2,y2_target,split,rstate):
    ...
    snpdata2 = preprocessing.scale(npdata2)
    X2_train, X2_test, y2_train, y2_test = train_test_split(snpdata2, nptarget2,
